...
import { Home, Circle, LogOut, Menu, X } from 'lucide-react';
...
const TABS = [
  ...
  { id: 'cs-product', label: 'CS & Product', icon: Circle },
  { id: 'sales-marketing', label: 'Sales & Marketing', icon: Circle },
  { id: 'finance', label: 'Finance', icon: Circle },
  { id: 'manager', label: 'Manager', icon: Circle },
  { id: 'data-engineering', label: 'Data Engineering', icon: Circle },
  { id: 'project-scoping', label: 'Project Scoping', icon: Circle },
  { id: 'project-planning', label: 'Project Planning', icon: Circle },
];
...
export default function Dashboard({ onLogout }) {
  ...
  return (
    <div className="min-h-screen flex bg-dark-bg">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-60 bg-dark-surface border-r border-dark-border
                         flex flex-col transition-transform lg:translate-x-0
                         ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-dark-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-accent-green to-accent-mint rounded-lg flex items-center justify-center">
              <span className="text-dark-bg font-bold text-xs">EW</span>
            </div>
            <span className="font-semibold text-text-primary text-sm">{"Ethan's Notes"}</span>
          </div>
            ...
            return (
              ...
              >
                {isHome ? (
                  ...
                  <Circle size={8} className={isActive ? 'fill-accent-green text-accent-green' : 'fill-text-muted text-text-muted'} />
                )}
                <span>{tab.label}</span>
              </button>
            );
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-hidden">
        <header className="h-12 bg-dark-surface border-b border-dark-border flex items-center justify-between px-4 lg:px-6 shrink-0">
          <div className="flex items-center gap-3">
